...
// Constructor
zc_calendar::zc_calendar(int X, int Y) :
	...
	value_(nullptr)
{
	...
	// Row 2 - the calendar
	table_ = new zc_calendar_table(C0, R1, WTABLE, H1, display_date_);
...
void zc_calendar::cb_bn_cal(Fl_Widget* w, void* v) {
	...
	case BN_OK:
		...
		char temp[25];
		strftime(temp, 25, that->format_, &that->table_->value());
		that->value_ = temp;
...
void zc_calendar::value(const char* date) {
	// Set the date
	value_ = date;
	// Update associated 
...
// Set format
void zc_calendar::format(const char* value) {
	format_ = value;
}

// Get format
const char* zc_calendar::format() {
	return format_;
}

...
zc_input_hierch::zc_input_hierch(int X, int Y, int W, int H, const char* L) :
...
zc_input_hierch::~zc_input_hierch()
{

}

// Overload value
const char* zc_input_hierch::value() {
...
// Overload value
void zc_input_hierch::value(const char* val) {
	// Set the input value
	input()->value(val);
...
void zc_input_hierch::add(const char* val) {
	const char* s = hierise(val);
...
void zc_input_hierch::hierarchy(int start, int end) {
	end_ = end;
	start_ = start;
}

// Get the hierarchy description
void zc_input_hierch::hierarchy(int* start, int* end) const {
	*end = end_;
	*start = start_;
}

const char* zc_input_hierch::hierise(const char* val) const {
	int len = end_ * end_;
	char* s = new char[len];
	memset(s, 0, len);

...
graph::graph(int X, int Y, int W, int H, const char* L) :
	Fl_Widget(X, Y, W, H, L)
{
	data_ = nullptr;
	set_drawing_area();
...
void graph::set_params(const options_t& x_options, const options_t& y_options) 
{
...
void graph::set_ticks(const options_t& options, std::vector<tick_t>& ticks, std::string& label) {
...
void graph::set_data(std::vector<coord>* data) {
	data_ = data;
//	convert_data_to_points();
}

//! \brief. Convert data point \p f from float to y position
int graph::float_to_point(float f, const options_t& options) {
...
void graph::normalise(float fin, float& norm, float& exp10, int& si_prefix, const axis_xier_t& xier) {
	exp10 = 1.0F;
	norm = fabs(fin);
	float step = 10.0F;
...
//void graph::convert_data_to_points() {
...
bool graph::in_drawing_area(int px, int py) {

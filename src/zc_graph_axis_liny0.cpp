...
//! \brief Constructor
zc_graph_axis_liny0::zc_graph_axis_liny0(int X, int Y, int W, int H, const char* L) :
	zc_graph_axis_linear(X, Y, W, H, L) {
}

//! \brief Draw the line for the axis.
void zc_graph_axis_liny0::draw_axis_line() {
...
void zc_graph_axis_liny0::draw_ticks() {
	...
	for (const auto& tick : ticks_) {
		...
		case zc_graph_axis::RIGHTWARDS:
			...
			fl_draw(tick.label.c_str(), x() + 5, ty + th / 2);
			break;
		default:
			break; // Do not draw if tick direction is invalid.
		}
...
void zc_graph_axis_liny0::draw_label() {

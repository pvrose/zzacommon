...
namespace zc {
	...
	struct lat_long_t {
		double latitude{ 0.0 };    //!< latitude (positive = North)
		double longitude{ 0.0 };   //!< longitude (positive = East)
	...
	//! radius of earth in kilometres - yes I know the earth is an oblate sphere, but...
	...
	size_t find(const char* data, size_t length, const char match);
	//! Returns the position of tring \p match (\p len_substr) in \p data (\p length): 
	//! returns \p length if not found. 
	size_t find_substr(const char* data, size_t length, const char* match, size_t len_substr);
	//! Returns the position of any characters not in \p match in \p data (\p length): 
	//! returns \p length if not found. 
	...
	*/
	...
	//! \brief Performs "XOR" encrypt/decrypt.	 
	...
	//! \param seed Seed to generate a pseudo-random sequence of characters
	//! to use as an encryption key.

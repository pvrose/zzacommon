...
enum file_data_t : uint8_t {
	FDD_REF_WORKING,               //!< Working directory for reference data
	FDD_REF_SOURCE,                //!< Original source directory for reference data
	FDD_DOCUMENTS,                 //!< Directory for HTML & PDF (userguide and code docs)
};
...
//! will be kept in this directory: other data will be moved to FDD_REF_WORKING.
...
//! This location is the same as FDD_REF_SOURCE.
//! 
class zc_file_holder
{
public:
	...
	*/
	zc_file_holder(const char* arg0, const std::map<uint8_t, file_control_t>& control);
	...
	std::string get_filename(uint8_t type) {
		...
		else return default_ref_directory_ + ctrl.filename;
	}

	//! Get directory for data \p type.
	std::string get_directory(file_data_t type) {
		switch (type) {
		case FDD_REF_WORKING: return default_ref_directory_;
		case FDD_REF_SOURCE: return default_source_directory_;
		case FDD_DOCUMENTS: return default_html_directory_;
		default: return "";
		}
	}
